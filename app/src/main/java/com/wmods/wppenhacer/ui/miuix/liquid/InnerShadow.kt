...
// Adapted from YuKongA/Mishka and Kyant0/AndroidLiquidGlass.
...
internal fun Modifier.innerShadow(shape: Shape, shadow: () -> InnerShadow?): Modifier =
...
private class InnerShadowElement(
    ...
    override fun create() = InnerShadowNode(shape, shadow)
...
private class InnerShadowNode(
    ...
    override fun ContentDrawScope.draw() {
        drawContent()
        val current = shadow() ?: return
        val layer = shadowLayer ?: return
        val radius = current.radius.toPx()
        // Resolve Dp values in the outer ContentDrawScope. Calling toPx() from the
        // GraphicsLayer recording scope makes Density delegate back into itself on
        // Compose 1.11 and eventually overflows the main-thread stack.
        val offsetX = current.offset.x.toPx()
        val offsetY = current.offset.y.toPx()
        ...
        layer.alpha = current.alpha
        layer.blendMode = current.blendMode
